...
## Validation croisée et tests placebo

La k-fold classique est inapplicable ici (données temporelles ; et la stratégie n'ayant aucun paramètre entraîné, un k-fold se réduirait à un bootstrap, déjà fait). Ses analogues corrects : sélection **walk-forward** du seuil EV (chaque saison n'utilise que le passé — converge vers les seuils bas, +115 k€ OOS à 100 €/pari) ; **leave-one-league-out** : en retirant n'importe quelle ligue, le ROI du reste tient entre +4.4 % et +5.4 % — aucune ligue ne porte le résultat. Tests placebo (1X2 @Max) : les paris à EV < −0.02 perdent −3.8 %, la zone neutre fait −0.5 %, la stratégie +4.8 % — gradient monotone parfait ; une sélection aléatoire @Max sur les mêmes matchs donne −1.0 % (le premium Max seul ne suffit pas : c'est bien la sélection par EV qui crée l'edge). Sensibilité au slippage : à cote Max −2 %, le seuil 0.02 domine tous les autres (+59 k€) ; à −3 %, les seuils < 0.01 deviennent perdants — le seuil optimal opérationnel est 0.02, à recalibrer sur la qualité d'exécution mesurée en paper trading.
