...
auc = [0.548, 0.535, 0.554, 0.535]
ax.bar(range(4), auc, color=[RED, ACCENT, ACCENT, ACCENT], edgecolor="#30363D", width=0.55, alpha=0.9)
ax.axhline(0.50, color="#8B949E", ls="--", lw=1, alpha=0.6)
ax.axhspan(0.58, 0.62, color=GREEN, alpha=0.07)
ax.text(3.4, 0.595, "Profitable zone (AUC >= 0.58)", fontsize=9, color=GREEN, alpha=0.6, ha="right")
ax.text(3.4, 0.503, "Random", fontsize=9, color="#8B949E", alpha=0.6, ha="right")
for i, v in enumerate(auc):
    ax.text(i, v + 0.003, f"{v:.3f}", ha="center", fontsize=13, fontweight="bold", color=[RED, ACCENT, ACCENT, ACCENT][i])
ax.set_xticks(range(4)); ax.set_xticklabels(versions, fontsize=10)
ax.set_ylim(0.48, 0.62); ax.set_ylabel("AUC (out-of-sample)")
ax.set_title("Discriminative Power Across 4 Iterations", fontsize=14, fontweight="bold", color="#F0F6FC", pad=15)
...
# 2. ROI
fig, ax = plt.subplots(figsize=(10, 5))
roi = [-7.91, -8.68, -6.56, -4.49]
ax.bar(range(4), roi, color=RED, edgecolor="#30363D", width=0.55, alpha=0.9)
...
ax.annotate("", xy=(3, -4.49), xytext=(0, -7.91), arrowprops=dict(arrowstyle="->", color=YELLOW, lw=2, ls="--"))
ax.text(1.5, -3, "Improving but never profitable", fontsize=10, color=YELLOW, ha="center", style="italic", alpha=0.8)
ax.set_xticks(range(4)); ax.set_xticklabels(versions, fontsize=10)
...
fig, ax = plt.subplots(figsize=(10, 5))
feats = ["mkt_dispersion", "max_avg_ratio", "comb_goals_r10", "comb_def_sot_r5", "mkt_range", "comb_sot_r5", "ou_margin", "comb_under_r5", "rank_diff", "h_is_under_r5"]
imp = [0.051, 0.050, 0.049, 0.049, 0.046, 0.046, 0.045, 0.043, 0.041, 0.039]
cf = [ORANGE if "mkt" in f or "max_avg" in f or "margin" in f else ACCENT for f in feats]
...
ax.set_title("Market Features Dominate Statistical Features", fontsize=14, fontweight="bold", color="#F0F6FC", pad=15)
...
lg = ["Bundesliga", "Premier League", "Ligue 1", "Serie A", "La Liga"]
rl = [-2.3, -9.9, -0.4, 4.2, -13.1]
...
plt.savefig(f"{OUT}/06_leagues.png", dpi=150, facecolor="#0D1117", bbox_inches="tight"); plt.close()
print("06_leagues.png")
